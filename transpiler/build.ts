...
console.log(chalk.blueBright(chalk.bold(`Transpiling ${chalk.redBright("plugin.lua")}...`)));
transpiler(false, true, "production");

const versionNumber = `v${fs.readFileSync(".version", "utf-8")}`;

console.log(chalk.blueBright(chalk.bold(`Modfifying version number of ${chalk.redBright("plugin.lua")}...`)));
...
console.log(chalk.blueBright(chalk.bold(`Updating ${chalk.redBright("README.md")}...`)));

const readme = fs.readFileSync("README.md", "utf-8");

if (readme.split("\n")[0].replaceAll("\r", "").split("plumoguSV ")[1] !== versionNumber) {
    const readmeLines = readme.split("\n");
    readmeLines[0] = `# plumoguSV ${versionNumber}`;
    fs.writeFileSync("README.md", readmeLines.join("\n"));
}

console.log(chalk.blueBright(chalk.bold(`Updating ${chalk.redBright("package.json")}...`)));
...
const packageVer = packageJSON.replaceAll("\r", "").split("\n")[packageIdx].split(': "')[1].replaceAll(/\"\,/g, "");

if (packageVer !== versionNumber.slice(1)) {
    const packageLines = packageJSON.split("\n");
    packageLines[packageIdx] = packageLines[packageIdx].replace(packageVer, versionNumber.slice(1));
    fs.writeFileSync("package.json", packageLines.join("\n"));
}

console.log(chalk.blueBright(chalk.bold(`Updating ${chalk.redBright("settings.ini")}...`)));
...
try {
    ...
    fs.writeFileSync("plugin.lua", `DISTRO="github"\n${pluginLines.join("\n")}`);
    fs.copyFileSync("plugin.lua", `temp-build-github/${packageName}/plugin.lua`);
    fs.writeFileSync("plugin.lua", `DISTRO="steam"\n${pluginLines.join("\n")}`);
    fs.copyFileSync("plugin.lua", `temp-build-steam/${packageName}/plugin.lua`);
    fs.copyFileSync("assets/steam_workshop_preview.png", `temp-build-steam/${packageName}/steam_workshop_preview.png`);
    fs.copyFileSync("assets/steam_workshop_id.txt", `temp-build-steam/${packageName}/steam_workshop_id.txt`);

    const settingsIni = fs.readFileSync("settings.ini", "utf-8");
    const oldVersionNumber = settingsIni.split("\n")[0].replaceAll("\r", "").split("plumoguSV")[1];
    ...
    if (!oldVersionNumber || oldVersionNumber !== versionNumber) {
        ...
        fs.writeFileSync(`temp-build-github/${packageName}/settings.ini`, settingsIniLines.join("\n"));
        fs.writeFileSync(`temp-build-steam/${packageName}/settings.ini`, settingsIniLines.join("\n"));
    }

    zip.addLocalFolder("temp-build-github");
    zip.writeZip(`builds/${packageName}.zip`);

    fs.cpSync(`temp-build-steam/${packageName}`, `builds/${packageName}-steam`, {
        recursive: true,
        force: true,
    });
