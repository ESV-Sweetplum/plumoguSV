function showYourSecondEffectTutorial()
    ...
    imgui.TextColored(INSTRUCTION_COLOR, 'Select "SPECIAL" under the "TYPE" dropdown.')
    ...
    ForceHeight(490)
    if
        not settingVars.linearlyChange and (math.abs(settingVars.mainSV - 0.2) > 0.001 or settingVars.svPercent ~= 80)
    then
        return
    end
    ForceHeight(450)
    ...
    imgui.TextColored(INSTRUCTION_COLOR, 'Enable "Change Stutter Over Time".')
    ForceHeight(640)
    ...
    ForceHeight(820)
    if
        math.abs(settingVars.mainSV) > 0.001
        or math.abs(settingVars.mainSV2 - 1) > 0.001
        or settingVars.svPercent ~= 100
        or settingVars.svPercent2 ~= 0
    then
        return
    end
    ForceHeight(800)
