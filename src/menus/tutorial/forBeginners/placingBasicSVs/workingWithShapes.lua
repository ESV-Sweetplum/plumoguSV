function showWorkingWithShapesTutorial()
    ...
    imgui.TextColored(INSTRUCTION_COLOR, 'Select "STANDARD" under the "TYPE" dropdown.')
    ...
    imgui.TextColored(INSTRUCTION_COLOR, 'Under the "STANDARD" tab, select "EXPONENTIAL".')
    ...
    if
        ...
        or math.abs(settingVars.startSV + 1.5) > 0.001
        or math.abs(settingVars.endSV - 1.5) > 0.001
    then
        return
    end
    ForceHeight(490)
    ...
    gpsim(
        'Working With Shapes Jumping',
        vctr2(1),
        function(t) return 0.9 - 2 * (t - t ^ 2) end,
        { { 1, 2, 3, 4 }, {}, {}, {} },
        500
    )
    imgui.TextColored(
