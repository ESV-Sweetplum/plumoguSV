function showKeybindSettings()
    ...
    for trueIdx, hotkeySettingNumber in pairs(HOTKEY_SETTING_ORDER) do
        ...
        if
            imgui.Button(
                awaitingIndex == hotkeyIndex and 'Listening...##keybind'
                    or hotkeyCombo:fixToSize(70) .. '##' .. hotkeyIndex
            )
        then
        ...
        if keyOccurrences > 1 then
            HoverToolTip(
                'This key is used in multiple keybind options:\n\n- '
