function checkForActionWheel()
    ...
    actionWheelData.size = actionWheelData.size
        + state.DeltaTime
            * 5.75
            / TARGET_ACTION_WHEEL_EXPANSION_TIME
            * (TARGET_ACTION_WHEEL_SIZE - actionWheelData.size)
    ...
    if actionWheelData.size < 1 then actionWheelData.size = 0 end
    ...
    ctx.AddCircle(
        actionWheelData.pos or vctr2(0),
        actionWheelData.size - BORDER_WIDTH / 2,
        imgui.GetColorU32(imgui_col.ButtonActive),
        128,
        BORDER_WIDTH
    )
    ctx.AddCircleFilled(
        actionWheelData.pos or vctr2(0),
        actionWheelData.size - BORDER_WIDTH,
    ...
    local col = color.vrgbaToUint(
        color.uintToRgba(imgui.GetColorU32(imgui_col.PlotHistogramHovered)) * vector.New(1, 1, 1, progress)
    )
