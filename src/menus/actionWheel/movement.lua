function kusaMovementDisplay(pos, size, selectedSegment)
    ...
    ctx.AddLine(topCenter - vector.New(size / 18, 0) * upDownScalingFactor,
        topCenter - vector.New(5 * size / 18, 0) * upDownScalingFactor, topColor,
        lineThickness)
    ctx.AddLine(topCenter - vector.New(size / 6, size / 9) * upDownScalingFactor,
        topCenter - vector.New(size / 6, -size / 9) * upDownScalingFactor,
        topColor,
        lineThickness)

    ctx.AddLine(topCenter + vector.New(size / 6, -size / 6) * upDownScalingFactor,
        topCenter + vector.New(size / 6, size / 6) * upDownScalingFactor,
        topColor,
        lineThickness)
    ctx.AddLine(topCenter + vector.New(size / 18, size / 6) * upDownScalingFactor,
        topCenter + vector.New(5 * size / 18, size / 6) * upDownScalingFactor,
        topColor,
        lineThickness)
    ctx.AddLine(topCenter + vector.New(size / 18, -size / 12) * upDownScalingFactor,
    ...
    ctx.AddLine(rightCenter + vector.New(0, size / 8) * leftRightScalingFactor,
        rightCenter + vector.New(size / 8, 0) * leftRightScalingFactor, rightColor, lineThickness)
    ctx.AddLine(rightCenter - vector.New(0, size / 8) * leftRightScalingFactor,
        rightCenter + vector.New(size / 8, 0) * leftRightScalingFactor, rightColor, lineThickness)

    -- Bottom Display

    ctx.AddLine(bottomCenter - vector.New(size / 18, 0) * upDownScalingFactor,
        bottomCenter - vector.New(5 * size / 18, 0) * upDownScalingFactor, bottomColor,
        lineThickness)
    ctx.AddLine(bottomCenter + vector.New(size / 6, -size / 6) * upDownScalingFactor,
        bottomCenter + vector.New(size / 6, size / 6) * upDownScalingFactor,
        bottomColor,
        lineThickness)
    ctx.AddLine(bottomCenter + vector.New(size / 18, size / 6) * upDownScalingFactor,
        bottomCenter + vector.New(5 * size / 18, size / 6) * upDownScalingFactor,
        bottomColor,
        lineThickness)
    ctx.AddLine(bottomCenter + vector.New(size / 18, -size / 12) * upDownScalingFactor,
    ...
    ctx.AddLine(leftCenter + vector.New(0, size / 8) * leftRightScalingFactor,
        leftCenter - vector.New(size / 8, 0) * leftRightScalingFactor, leftColor, lineThickness)
    ctx.AddLine(leftCenter - vector.New(0, size / 8) * leftRightScalingFactor,
        leftCenter - vector.New(size / 8, 0) * leftRightScalingFactor, leftColor, lineThickness)
end
