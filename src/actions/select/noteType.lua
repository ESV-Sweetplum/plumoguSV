...
function selectByNoteType(menuVars)
    ...
    for _, note in ipairs(totalNotes) do
        if hitobject_type then
            if note.Type == hitobject_type.Normal and not menuVars.normal then goto skipType end
            if note.Type == hitobject_type.Mine and not menuVars.mine then goto skipType end
