function placeTeleportStutterSVs(settingVars)
    ...
    for i = 1, numTeleportSets do
        ...
        for j = 0, settingVars.stuttersPerSection - 1 do
            ...
            if sv2 ~= sv1 then addSVToList(svsToAdd, currentStart + startDuration, sv2, true) end
            if sv3 ~= sv2 then
                addSVToList(svsToAdd, currentEnd - endDuration, sv3 / settingVars.stuttersPerSection, true)
            end
        end
    end
    local finalMultiplier = settingVars.avgSV
    if finalSVType ~= 'Normal' then finalMultiplier = settingVars.customSV end
    if finalSVType ~= 'None' then addFinalSV(svsToAdd, lastOffset, finalMultiplier, finalSVType == 'Override') end
    removeAndAddSVs(svsToRemove, svsToAdd)
end
