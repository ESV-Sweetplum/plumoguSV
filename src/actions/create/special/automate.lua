...
function clearAutomateSVs(settingVars) settingVars.copiedSVs = {} end
...
function automateSVs(settingVars)
    ...
    for idx, ho in pairs(selected) do
        ...
        do -- avoid jumping over local scope error
            ...
            local prevTime = truthy(selected[math.max(1, idx - 1)].EndTime) and selected[math.max(1, idx - 1)].EndTime
                or selected[math.max(1, idx - 1)].StartTime
            ...
            if not neededIds[idName] then neededIds[idName] = { hos = {}, svs = {} } end
            ...
            for _, sv in ipairs(settingVars.copiedSVs) do
                ...
                if settingVars.scaleSVs then
                    local scalingFactor = (currentTime - startTime) / (secondaryTime - startTime)
