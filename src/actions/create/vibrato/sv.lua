...
function svVibrato(menuVars, heightFn)
    ...
    local fps = globalVars.vibratoFrameRates[menuVars.vibratoQuality] + 0.69
    ...
    for i = 1, #offsets - 1 do
        ...
        if menuVars.sides == 1 then
            for tp = 1, teleportCount, 2 do
                ...
                local height = heightFn(math.floor((tp - 1) / 2) * 2 / teleportCount * posDifference + startPos, tp)
                height = deviateVibratoHeight(height, menuVars.deviationFunctionIndex, menuVars.deviationDistance)
                prepareDisplacingSVs(offset, svsToAdd, svTimeIsAdded, nil, height, 0)
                x = tp / teleportCount
        ...
        elseif menuVars.sides == 2 then
            ...
            prepareDisplacingSVs(startVibro, svsToAdd, svTimeIsAdded, nil, -heightFn(startPos, 0), 0)
            local prevHeight = heightFn(startPos, 0)
            for tp = 1, teleportCount - 2, 2 do
                local x = tp / teleportCount
                local offset = nextVibro * x + startVibro * (1 - x)
                local newHeight = heightFn((tp + 1) / (teleportCount - 1) * posDifference + startPos, tp + 1)
            ...
            for tp = 1, teleportCount - 2, 3 do
                ...
                newHeight = deviateVibratoHeight(newHeight, menuVars.deviationFunctionIndex, menuVars.deviationDistance)
                prepareDisplacingSVs(offset, svsToAdd, svTimeIsAdded, nil, -prevHeight, 0)
                x = tp / teleportCount
                offset = nextVibro * x + startVibro * (1 - x)
                prepareDisplacingSVs(offset, svsToAdd, svTimeIsAdded, nil, prevHeight + newHeight, 0)
                x = (tp + 1) / teleportCount
                offset = nextVibro * x + startVibro * (1 - x)
                prepareDisplacingSVs(offset, svsToAdd, svTimeIsAdded, nil, -newHeight, 0)
                prevHeight = newHeight
            end
        end
    end
