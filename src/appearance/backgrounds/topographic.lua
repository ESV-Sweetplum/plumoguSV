function renderTopographicMap()
    ...
    for y = 0, yNodeCount - 1 do
        for x = 0, xNodeCount - 1 do
            if map[y + 1][x + 1] < 0 then
                ctx.AddCircleFilled(
                    graphTopLeft + CELL_SIZE * vector.New(x, y),
                    2,
    ...
    for y = 1, yCellCount do
        for x = 1, xCellCount do
            ...
            local topPoint = graphTopLeft + CELL_SIZE * vector.New(x - 1 + topZeroLocation, y - 1)
            local bottomPoint = graphTopLeft + CELL_SIZE * vector.New(x - 1 + bottomZeroLocation, y)
...
function perlin.noise(x, y, z)
    ...
    return perlin.lerp(
        ...
        perlin.lerp(
            v,
            perlin.lerp(u, perlin.grad(AAA, x, y, z), perlin.grad(BAA, x - 1, y, z)),
            perlin.lerp(u, perlin.grad(ABA, x, y - 1, z), perlin.grad(BBA, x - 1, y - 1, z))
        ),
        perlin.lerp(
            v,
            perlin.lerp(u, perlin.grad(AAB, x, y, z - 1), perlin.grad(BAB, x - 1, y, z - 1)),
            perlin.lerp(u, perlin.grad(ABB, x, y - 1, z - 1), perlin.grad(BBB, x - 1, y - 1, z - 1))
        )
    )
end
...
function perlin.grad(hash, x, y, z) return perlin.dot_product[bit32.band(hash, 0xF)](x, y, z) end
