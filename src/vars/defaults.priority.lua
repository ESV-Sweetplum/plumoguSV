function loadDefaultProperties(defaultProperties)
    ...
    for label, tbl in pairs(defaultProperties.menu) do
        for settingName, settingValue in pairs(tbl) do
            ...
            if defaultSetting == nil then goto nextSetting end
...
function parseDefaultProperty(v, default)
    if default == nil or type(default) == 'table' or type(default) == 'userdata' then return nil end
    if type(default) == 'number' then return tn(v) end
    if type(default) == 'boolean' then return truthy(v) end
    if type(default) == 'string' then return v end
    return v
end
