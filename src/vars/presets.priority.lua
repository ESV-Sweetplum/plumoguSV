...
function checkPresetValidity(preset)
    ...
    local validMenus = {}
    if preset.type == 'Standard' or preset.type == 'Still' then validMenus = table.duplicate(STANDARD_SVS) end
    if preset.type == 'Special' then validMenus = table.duplicate(SPECIAL_SVS) end
    if preset.type == 'Vibrato' then validMenus = table.duplicate(VIBRATO_SVS) end
